...
static void read_system_config(void) {
    LOGGED_FUNCTION_START_NO_PARAMS
        debug("Reading memory management properties...");
        uint32_t page_size, allocation_granularity;
            ...
            if (info.dwAllocationGranularity > (DWORD)UINT32_MAX) {
                exit_after_c_error(
                    "That's an unexpectedly big allocation granularity!"
                );
            }
            allocation_granularity = info.dwAllocationGranularity;

            debug("Reading current process pseudo handle...");
        ...
        infof("OS kernel allocates memory at a granularity of %u (%#x) bytes.",
              allocation_granularity, allocation_granularity);
        assert(allocation_granularity >= page_size);
        system_allocation_granularity_pow2 =
            pow2_encode(allocation_granularity);
    LOGGED_FUNCTION_END
}
...
static size_t allocation_size(size_t size) {
    LOGGED_FUNCTION_START("%#zx", size)
        ...
        if (trailing_bytes != 0) {
            ...
            debugf("Rounded allocation size up to %zu (%#zx) bytes.",
                   size, size);
        }
        return size;
    LOGGED_FUNCTION_END
...
static bool try_increase_mlock_budget(size_t size) {
    LOGGED_FUNCTION_START("%#zx", size)
        debug("Locking memory locking limit mutex...");
        ...
        bool result = false;
        debug("Querying initial memory locking limit...");
        // RLIMIT_MEMLOCK is a Linux/BSD thing whose broader support is unknown,
        // add support for other OSes as needed after checking they do support
        // it and that they use the same errno logic.
        #ifdef __linux__
            ...
            debugf("Current memory locking limit is %zu/%zu bytes",
            ...
            if (mlock_limit.rlim_cur > mlock_limit.rlim_max) {
                if (mlock_limit.rlim_max - initial_cur >= size) {
                    // If this gets us above the hard limit but the hard limit
                    // is high enough, then better saturate at the hard limit.
                    mlock_limit.rlim_cur = mlock_limit.rlim_max;
                } else {
                    // Otherwise we have no choice but to try increasing the
                    // hard limit (will fail if the process is not privileged).
                    mlock_limit.rlim_max = mlock_limit.rlim_cur;
                }
            }
            debugf("Will attempt to raise the limit to %zu/%zu bytes",
                   (size_t)mlock_limit.rlim_cur, (size_t)mlock_limit.rlim_max);

            if (setrlimit(RLIMIT_MEMLOCK, &mlock_limit) == 0) {
                debug("Successfully raised the memory locking limit.");
                result = true;
            ...
            // address space.
            case EFAULT:
            // The value specified in resource is not valid or rlim->rlim_cur
            ...
            case EPERM:
                ...
                warn("Failed to raise the hard memory locking limit. Please "
                     "raise the memory locking limit for this user/group "
            ...
            debugf("Current process working set size is %zu/%zu bytes.",
            ...
            debugf("Will attempt to increase the working set to %zu/%zu bytes.",
                   min_working_set, max_working_set);
...
void* realtime_allocate(size_t size) {
    LOGGED_FUNCTION_START("%#zx", size)
        debugf("Checking and rounding requested allocation size %#zx...", size);
        ...
        const char* mlock_failure_msg =
            ...
            if (result == MAP_FAILED) {
                exit_after_c_error("Failed to allocate memory!");
            }
            debugf("Allocated memory pages at virtual location %p.", result);
            assert((size_t)result % page_size == 0);

            debug("Now, let's try to lock allocated pages into RAM...");
            if (mlock(result, size) == 0) {
                debug("mlock() succeeded on first try.");
                goto log_and_return;
            }
            switch (errno) {
            // Either addr+size overflows or addr is not aligned to a page
            case EINVAL:
                exit_after_c_error("Cannot happen if mmap() works correctly!");
            // This can mean several different things:
            // - Not in process address space (impossible if mmap() works)
            // - Maximal number of memory mappings exceeded
            // - RLIMIT_MEMLOCK soft limit exceeded
            case ENOMEM:
                errno = 0;
                debug("Failed to lock memory, may have hit a soft limit. "
            ...
            if (mlock(result, size) == 0) {
                debug("mlock() succeeded after raising the rlimit.");
                goto log_and_return;
            }
            warn_on_errno();
            warn(mlock_failure_msg);
            ...
            if (VirtualLock(result, size) == 0) {
                debug("VirtualLock() succeeded on first try.");
                goto log_and_return;
            }
            win32_warn_on_error();
            debug("Failed to lock memory, but maybe the process working set "
                  "is too small. Try to raise it before giving up...");
            ...
            // If mlock fails again after adjusting the rlimit, then give up
            if (VirtualLock(result, size) == 0) {
                debug("VirtualLock() succeeded after raising the working set.");
                goto log_and_return;
            }
            win32_warn_on_error();
            warn(mlock_failure_msg);
    ...
    prefault_and_return:
        debug("If we can't lock our memory, at least pre-fault it...");
...
void realtime_liberate(void* buffer, size_t size) {
    LOGGED_FUNCTION_START("%p, %#zx", buffer, size)
        debugf("Rounding up user-specified size %#zx for allocation %p...",
               size, buffer);
        size = allocation_size(size);

        #ifndef NDEBUG
            debug("Zeroing liberated buffer to help detect more bugs...");
            memset(buffer, 0, size);
        #endif

        // Note that neither code path decreases RLIMIT_MEMLOCK (Unix) or the
        // process working set size (Windows). While this is obviously meh from
        // a "telling the OS kernel the whole truth" perspective, it still feels
        // like the right move for the following reasons:
        //
        // - We don't increase the limit if we don't need to, so this is not a
        //   resource leak where the limit keeps increasing indefinitely. We're
        //   merely keeping the limit at our maximal resource usage so far,
        //   which is a (possibly bad) upper bound on our actual resource usage.
        // - If we decrease the limit when an allocation is liberated, then we
        //   need to increase it again when we allocate again, so every
        ...
        //   decrease the limit, it should eventually converge to a correct
        //   upper bound that doesn't need adjusting anymorre.
        // - By avoiding limit-setting syscalls in the long run, we also reduce
        //   the risk of associated race conditions, which is also good, though
        //   it would obviously be better if POSIX and Windows limit-adjustment
        //   syscalls weren't racy by design...
        #ifdef __unix__
    ...
    static void check_allocation_size(size_t size) {
        LOGGED_FUNCTION_START("%#zx", size)
            ...
            debugf("Allocated memory at address %p.", raw_alloc);
            ...
            debug("Liberating the allocation...");
