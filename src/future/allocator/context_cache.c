...
future_context_cache_t future_context_cache_initialize() {
    LOGGED_FUNCTION_START_NO_PARAMS
        ...
        debugf("...done, thread cache array is @ %p with capacity %zu.",
...
void future_context_cache_register_thread(future_context_cache_t* context_cache,
    ...
    LOGGED_FUNCTION_START("%p, %p", context_cache, thread_cache)
        debug("Locking context cache mutex...");
        ...
        if (
            ...
            debugf("Growing thread cache list %p which has outgrown "
                   "its current capacity %zu...",
            ...
            if (context_cache->thread_caches == NULL) {
                ...
                exit_after_c_error("Failed to grow the thread caches array");
            }
        ...
        debugf("Will insert thread cache at index %zu of "
               "the context cache's list.",
               index);
...
void future_context_cache_finalize_threads(future_context_cache_t* cache) {
    LOGGED_FUNCTION_START("%p", cache)
        // Notice that cache->mutex is not taken here. This is done deliberately
        // in order to avoid the deadlock scenario discussed in the
        // thread_caches member's documentation. Associated race conditions are
        // handled in the manner that is described in said documentation.
