...
//! \brief Inner file descriptor of \ref udipe_future_t
    ...
    /// long-lived internal file descriptor which can be...
    ///
    /// - An \ref inpoll_t attached to the upstream futures (unordered).
    /// - A repeating timerfd (repeating timer).
    ///
    ...
    ///   destroyed, if we want to keep inpoll-based waiting easy and effective.
    ///
    /// Instead, the `status_sync` fd of these "chained" futures is actually an
    /// \ref inpoll_t which is initially attached to two other file descriptors:
    ///
    ...
    ///   `status_sync.latched_inpoll`, letting client knowns that this future
    ///   is ready to make progress (and possibly change status as a result).
    ...
    ///   `status_sync.latched_inpoll` and reconfigured as appropriate for the
    ...
    ///   and the \ref inpoll_latch_event_t is signaled to notify clients that
    ///   this future has reached its final state.
    ///
    /// Whenever this pattern is used, this \ref fd_t typedef is used to
    /// annotate the internal fd that signals the event of interest, so that
    /// relevant documentation does not need to be replicated in two different
    /// places.
