...
///
/// Unordered and repeating timer operations emit a chain of futures. On Linux,
/// these child futures are generated based on information from a long-lived
/// internal file descriptor which is...
///
...
///
/// Whenever this pattern is used, this `int` typedef is used to annotate the
/// internal fd that signals the event of interest, so that relevant
/// documentation does not need to be replicated in two different places.
typedef int inner_fd_t;
