    ...
    distribution_t distribution_allocate(size_t capacity) {
        LOGGED_FUNCTION_START("%zu", capacity)
    ...
    void distribution_create_bin(distribution_builder_t* builder,
        ...
        LOGGED_FUNCTION_START("%p, %zu, %zd, %zu", builder, pos, value, count)
            ...
            if (dist->num_bins < dist->capacity) {
                debug("There's enough room for this new bin.");
    ...
    distribution_t distribution_build(distribution_builder_t* non_empty_builder) {
        LOGGED_FUNCTION_START("%p", non_empty_builder)
    ...
    distribution_t distribution_scale(distribution_builder_t* empty_builder,
        ...
        LOGGED_FUNCTION_START("%p, %zd, %p", empty_builder, factor, dist)
            ...
            for (size_t dist_pos = 0; dist_pos < num_bins; ++dist_pos) {
                ...
                assert(dist_end_rank > prev_end_rank);  // >0 count
                const size_t count = dist_end_rank - prev_end_rank;
                prev_end_rank = dist_end_rank;

                const size_t builder_pos =
                    (factor > 0) ? dist_pos : num_bins - dist_pos - 1;
    ...
    distribution_t distribution_sub(distribution_builder_t* empty_builder,
        ...
        LOGGED_FUNCTION_START("%p, %p, %p", empty_builder, minuend, subtrahend)
            ...
            // iterate over the shortest dist and sample from the longest one
            ...
            const distribution_layout_t short_layout =
                distribution_layout(shorter);
            ...
            for (size_t short_pos = 0; short_pos < short_bins; ++short_pos) {
                const int64_t short_value =
                    short_layout.sorted_values[short_pos];
                ...
                for (
                    size_t long_sample = 0;
                    long_sample < short_count;
                    ++long_sample
                ) {
                    const int64_t diff =
                        short_value - distribution_choose(longer);
                    tracef("  * Random short-long difference is %zd.", diff);
                    const int64_t signed_diff = diff_sign * diff;
                    tracef("  * Random minuend-subtrahend difference is %zd.",
                           signed_diff);
    ...
    distribution_t distribution_scaled_div(distribution_builder_t* empty_builder,
        ...
        LOGGED_FUNCTION_START("%p, %p, %zd, %p",
            ...
            if (distribution_len(num) <= distribution_len(num)) {
                ...
                const distribution_layout_t num_layout =
                    distribution_layout(num);
                ...
                for (size_t num_pos = 0; num_pos < num_bins; ++num_pos) {
                    ...
                    tracef("- Numerator bin #%zu contains "
                           "%zu occurences of value %zd.",
                           num_pos, num_count, num_value);
                    for (
                        size_t denom_sample = 0;
                        denom_sample < num_count;
                        ++denom_sample
                    ) {
                        const int64_t denom_value = distribution_choose(denom);
                        tracef("  * Sampled random denominator value %zd.",
                               denom_value);
                        const int64_t scaled_ratio =
                            num_value * factor / denom_value;
                ...
                const distribution_layout_t denom_layout =
                    distribution_layout(denom);
                const size_t denom_bins = denom->num_bins;
                debugf("Iterating over the %zu bins "
                       "of the denominator distribution...",
                       denom_bins);
                size_t prev_end_rank = 0;
                for (
                    size_t denom_pos = 0;
                    denom_pos < denom_bins;
                    ++denom_pos
                ) {
                    const int64_t denom_value =
                        denom_layout.sorted_values[denom_pos];
                    const size_t curr_end_rank =
                        denom_layout.end_ranks[denom_pos];
                    const size_t denom_count = curr_end_rank - prev_end_rank;
                    tracef("- Denominator bin #%zu contains "
                           "%zu occurences of value %zd.",
                           denom_pos, denom_count, denom_value);
                    for (
                        size_t num_sample = 0;
                        num_sample < denom_count;
                        ++num_sample
                    ) {
                        const int64_t num_value = distribution_choose(num);
                        tracef("  * Sampled random numerator value %zd.",
                               num_value);
                        const int64_t scaled_ratio =
                            num_value * factor / denom_value;
                        tracef("  * Scaled ratio sample is %zd.", scaled_ratio);
    ...
    uint64_t distribution_min_difference_with(const distribution_t* d1,
        ...
        LOGGED_FUNCTION_START("%p, %p", d1, d2)
            ...
            if (num_bins[0] == 0 || num_bins[1] == 0) {
                debug("At least one distribution is empty, "
                      "will return UINT64_MAX");
                return UINT64_MAX;
            }
            ...
            for (
                ...
                const int64_t short_value =
                    sorted_values[short_dist][short_bin];
                ...
                if (difference == 0) {
                    ...
                    if (long_bin > 0) {
                        difference = short_value
                                     - sorted_values[long_dist][long_bin - 1];
                        assert(difference > 0);
                    }
                    if (long_bin + 1 < num_bins[long_dist]) {
                        const int64_t diff_candidate =
                            sorted_values[long_dist][long_bin + 1]
                            - short_value;
                    ...
                    // No suitable neighbor -> No >0 distance from short_value
        ...
        static void test_distribution() {
            LOGGED_FUNCTION_START_NO_PARAMS
                ...
                const int64_t value5 =
                    value3 + 2 + rand() % (INT64_MAX - value3 - 1);
                ...
                const int64_t value1 =
                    value3 - 2 - rand() % (value3 - 1 - INT64_MIN);
                ...
                while (builder.inner.num_bins < builder.inner.capacity) {
                    trace("- No reallocation expected here. Backing up...");
                    memcpy(prev_data,
                    ...
                    trace("- Checking global info which shouldn't change...");
                    ensure_eq(builder.inner.allocation, initial_allocation);
                    ensure_eq(builder.inner.capacity, initial_capacity);
                    ...
                    if (insert_pos == SIZE_MAX) {
                        ...
                        ensure_eq(builder.inner.num_bins,
                                  prev_bins + after_offset);
                    }
                }
                ...
                for (size_t src_pos = 0; src_pos < prev_bins; ++src_pos) {
                    tracef("  * Checking former bin #%zu...", src_pos);
                    const size_t dst_pos =
                        src_pos + (size_t)(src_pos >= insert_pos);
                    ensure_eq(layout.sorted_values[dst_pos],
                              prev_values[src_pos]);
                    ensure_eq(layout.counts[dst_pos],
                              prev_counts[src_pos]);
                ...
                for (size_t i = 0; i < num_samples; ++i) {
                    ...
                    trace("- Checking const correctness "
                            "and locating sampled bin...");
                    size_t sampled_bin = SIZE_MAX;
                    for (size_t bin = 0; bin < dist.num_bins; ++bin) {
                        if (layout.sorted_values[bin] == sample) {
                            sampled_bin = bin;
                        }
                        ensure_eq(layout.sorted_values[bin], prev_values[bin]);
                ...
                // TODO: Break up in smaller functions + test newer
                //       functionality like scale, sub, scaled_div...
            LOGGED_FUNCTION_END
        }
