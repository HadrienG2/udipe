    ...
    distribution_pool_t distribution_pool_initialize() {
        LOGGED_FUNCTION_START_NO_PARAMS
            const size_t capacity =
                get_page_size() / sizeof(distribution_builder_t);
            ensure_ne(capacity, (size_t)0);
            void* const allocation =
                malloc(capacity * sizeof(distribution_builder_t));
            exit_on_null(allocation, "Failed to allocate distribution pool");
            debugf("Allocated distribution pool with %zu entries @ %p.",
                   capacity, allocation);
    ...
    distribution_builder_t distribution_pool_request(distribution_pool_t* pool) {
        LOGGED_FUNCTION_START("%p", pool)
            assert(pool->builders);
            if (pool->length >= 1) {
                --(pool->length);
                distribution_builder_t builder = pool->builders[pool->length];
                debugf("Successfully reused distribution @ %p.",
                       builder.inner.allocation);
                return builder;
            } else {
                debug("No distribution available, allocating a new one...");
    ...
    void distribution_pool_recycle(distribution_pool_t* pool,
                                   distribution_t* dist) {
        LOGGED_FUNCTION_START("%p, %p", pool, dist)
            assert(pool->builders);
            ...
            if (pool->length == pool->capacity) {
                ...
                exit_on_null(pool->builders,
                             "Failed to grow distribution pool");
                pool->capacity = new_capacity;
                if (pool->builders == old_builders) {
                    debugf("Grew distribution pool @ %p "
                           "to %zu entries (%zu bytes).",
    ...
    void distribution_pool_finalize(distribution_pool_t* pool) {
        LOGGED_FUNCTION_START("%p", pool)
            ...
            debugf("Liberating distribution pool storage @ %p...",
                   pool->builders);
