    ...
    void log_estimate(udipe_log_level_t level,
        ...
        LOGGED_FUNCTION_START(
            ...
            // Quantify the relative fluctuation with respect to sample value
            double rel_width =
                (estimate.high - estimate.low) / fabs(estimate.sample);
    ...
    void log_statistics(udipe_log_level_t level,
                        ...
                        statistics_t stats,
                        const char unit[]) {
        LOGGED_FUNCTION_START("%d, \"%s\", \"%s\", (statistics), \"%s\"",
                              level,
                              title,
                              bullet,
                              unit)
    ...
    analyzer_t analyzer_initialize() {
        LOGGED_FUNCTION_START_NO_PARAMS
            ...
            debug("Setting up resampling distribution builder...");
            ...
            analyzer.mean_accumulators =
                malloc(analyzer.mean_capacity * sizeof(double));
    ...
    statistics_t analyzer_apply(analyzer_t* analyzer,
        ...
        LOGGED_FUNCTION_START("%p, %p", analyzer, dist)
            ...
            for (size_t run = 0; run < NUM_RESAMPLES; ++run) {
                ...
                analyzer->statistics[MEAN][run] = analyze_mean(analyzer,
                                                               &resample);
                ...
                analyzer->statistics[CENTER_WIDTH][run] =
    ...
    void analyzer_finalize(analyzer_t* analyzer) {
        LOGGED_FUNCTION_START("%p", analyzer)
            debug("Destroying resample distribution builder...");
    ...
    size_t write_mean_difference(char output[],
                                 ...
                                 double sample_mean) {
        LOGGED_FUNCTION_START(
            "%p, %zu, { .sample = %f, .low = %f, .high = %f }, %d, %f",
            output,
            output_size,
            value.sample,
            value.low,
            ...
            case DELTA:
                const double rel_delta =
                    (value.sample - sample_mean) / fabs(sample_mean);
    ...
    double analyze_mean(analyzer_t* analyzer,
        ...
        LOGGED_FUNCTION_START("%p, %p", analyzer, dist)
            ...
            debugf("- Distribution contains %zu values, leading to norm %g.",
                   len, len_norm);
    ...
    void set_result_confidence(analyzer_t* analyzer,
                               statistic_id_t stat,
                               estimate_t* estimate) {
        LOGGED_FUNCTION_START("%p, %d, %p", analyzer, stat, estimate)
