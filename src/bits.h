...
/// Number of bits in each word of entropy
///
/// Must use 64-bit integers even on 32-bit systems otherwise we cannot easily
/// extract enough entropy to fill a double-precision number.
#define BITS_PER_ENTROPY_WORD  ((size_t)64)
...
/// Turn entropy words into random integers
///
/// Using a bunch of uniformly random words in `inputs` which were previously
/// generated by a method like generate_entropy(), extract output words in
/// `outputs` that each have `bits_per_output` low-order bits randomized and
/// higher-order bits set to zero.
///
/// \param bits_per_output indicates how many low-order bits of each word from
///                        `outputs` should be randomized.
/// \param outputs points to the first output element that should be filled.
/// \param num_outputs indicates how many output elements should be generated
///                    starting from `outputs[0]`.
/// \param consumed_input_bits should be initialized to zero, and will be
///                            updated on each call to this function to track
///                            how many bits of entropy were consumed so far.
/// \param inputs points to the first word of entropy from which random bits
///               can be extracted.
/// \param num_inputs indicates how many words of entropy starting from
///                   `inputs[0]` can be used as a source of rancom data.
UDIPE_NON_NULL_ARGS
