...
udipe_connect_result_t udipe_connect(udipe_context_t* context,
    ...
    udipe_future_t* const future = udipe_start_connect(context, options);
    assert(future);
...
udipe_disconnect_result_t udipe_disconnect(udipe_context_t* context,
    ...
    // FIXME: Do not force this synchronous implementation style, leave the
    //        choice to the backend.
    udipe_future_t* const future = udipe_start_disconnect(context, options);
    assert(future);
    const udipe_result_t result = udipe_finish(future);
...
udipe_send_result_t udipe_send(udipe_context_t* context,
    ...
    // FIXME: Do not force this synchronous implementation style, leave the
    //        choice to the backend.
    udipe_future_t* const future = udipe_start_send(context, options);
    assert(future);
    const udipe_result_t result = udipe_finish(future);
...
udipe_recv_result_t udipe_recv(udipe_context_t* context,
    ...
    // FIXME: Do not force this synchronous implementation style, leave the
    //        choice to the backend.
    udipe_future_t* const future = udipe_start_recv(context, options);
    assert(future);
    const udipe_result_t result = udipe_finish(future);
