...
udipe_result_t udipe_finish(udipe_future_t* future) {
    LOGGER_START(&future->context->logger)
        ...
        debug("Setting up result storage...");
        udipe_result_t result = { 0 };

        debug("Offloading the rest to the future_finish() backend...");
...
bool udipe_cancel(udipe_future_t* future, bool finish) {
    LOGGER_START(&future->context->logger)
        debugf("Loading current status of target future %p...", future);
        future_status_t status = future_status_load(
            future,
        ...
        debugf("Analyzing future type %u...", status.type);
    ...
    finish:
        if (finish) {
            debug("Asked to wait for completion, "
                  "will offload this to future_finish()...");
            future_finish(future, status, NULL);
        }
        return canceled;
    LOGGER_END
}
...
udipe_future_t* udipe_start_custom(udipe_context_t* context) {
    LOGGER_START(&context->logger)
        debugf("Allocating a custom future from context %p...", context);
        udipe_future_t* const future = future_allocate(context, TYPE_CUSTOM);

        debug("Checking its initial state in debug builds...");
        ...
        debug("Initializing the initial status word...");
...
void future_finish(udipe_future_t* future,
    ...
    LOGGED_FUNCTION_START(
        ...
        do {
            debugf("Checking the initial status of future %p...", future);
            ...
            debug("Marking the future as unavailable...");
            ...
            if (success) {
                debug("Done marking the future as unavailable.");
        ...
        if (latest_status.state == STATE_RESULT) {
            debug("Result was available from the start!");
        } else {
            debug("Waiting for the result to become available...");
        ...
        if (result) {
            debug("Collecting the end result...");
            *result = (udipe_result_t){ 0 };
            switch (latest_status.outcome) {
            case OUTCOME_SUCCESS:
            case OUTCOME_FAILURE_INTERNAL:
                debug("Future went far enough to produce a typed result");
                ...
                break;
            case OUTCOME_FAILURE_DEPENDENCY:
                debug("Future failed because one of its dependencies has failed.");
                result->type = UDIPE_FAILURE_DEPENDENCY;
                break;
            case OUTCOME_FAILURE_CANCELED:
                debug("Future failed because it was canceled.");
...
bool future_custom_check_canceled(future_status_t status) {
    LOGGED_FUNCTION_START(
        "{ .dc = %u, .dco = %u, .a = %u, .s = %u, .o = %u, .t = %u, .na = %u, "
          ".ne/ll = %u, %u }",
        status.downstream_count,
        status.downstream_count_overflow,
        status.available,
        ...
        debug("Checking the initial status...");
        future_status_debug_check(status, true);
        ensure_eq((future_type_t)status.type,
                  (future_type_t)TYPE_CUSTOM);

        #ifndef NDEBUG
            debug("Checking the outcome/state consistency...");
            switch (status.outcome) {
    ...
    static void custom_test_seq_success(udipe_context_t* context) {
        LOGGED_FUNCTION_START("%p", context)
            ...
            debug("Making sure it's not initially canceled...");
            ensure(!udipe_custom_canceled(custom));

            debug("Marking it as finished with some dummy payload...");
    ...
    static void custom_test_seq_failure(udipe_context_t* context) {
        LOGGED_FUNCTION_START("%p", context)
            ...
            debug("Marking it as failed with some dummy payload...");
            const udipe_custom_payload_t payload = generate_custom_payload();
            const bool success = udipe_custom_try_set_result(custom,
                                                             false,
                                                             payload);
    ...
    static void custom_test_seq_canceled_success(udipe_context_t* context) {
        LOGGED_FUNCTION_START("%p", context)
            ...
            debug("Canceling it without finishing execution "
                  "(would deadlock in this sequential run)...");
            const bool canceled = udipe_cancel(custom, false);
            ensure(canceled);

            debug("Confirming that it's marked as canceled");
            ensure(udipe_custom_canceled(custom));

            debug("Marking it as successful with some dummy payload...");
            const udipe_custom_payload_t payload = generate_custom_payload();
            const bool success = udipe_custom_try_set_result(custom,
                                                             true,
                                                             payload);
    ...
    static void custom_test_seq_canceled_acknowledged(
        ...
        LOGGED_FUNCTION_START("%p", context)
            ...
            debug("Observing that it's canceled...");
            ensure(udipe_custom_canceled(custom));

            debug("Acknowledging the cancelation...");
            udipe_custom_acknowledge_cancel(custom);
