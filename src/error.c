...
void warn_on_errno() {
    ...
    //          name ends with f). Basic static string logging is fine. This is
    //          why raw SCOPE_START is used instead of the usual
    //          LOGGED_FUNCTION_START, which calls into tracef.
    SCOPE_START
        // No errno, no output
        if (errno == 0) return;
    ...
    void win32_warn_on_error() {
        LOGGED_FUNCTION_START_NO_PARAMS
            ...
            // TODO: Print out the textual description using FormatMessage() as
            //       in the errno case instead of simply printing the numerical
            //       error code as this code currently does.
            //
            //       For now, please refer to the "Debug system error codes"
            //       MSDN page which provides a table of Windows error codes and
            //       some tools to automatically look them up:
